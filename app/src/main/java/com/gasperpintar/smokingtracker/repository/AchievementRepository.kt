...
class AchievementRepository(private val achievementDao: AchievementDao) {
    ...
    suspend fun insert(entry: AchievementEntity) {
        achievementDao.insert(entity = entry)
    }

    suspend fun insert(entries: List<AchievementEntity>) {
        achievementDao.insertAll(entities = entries)
    }

    suspend fun update(entry: AchievementEntity) {
        achievementDao.update(entity = entry)
    }

    suspend fun delete(entry: AchievementEntity) {
    ...
    suspend fun incrementAchievementTimes(achievementId: Long) {
