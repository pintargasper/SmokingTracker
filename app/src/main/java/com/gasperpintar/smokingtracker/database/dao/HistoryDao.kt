...
interface HistoryDao: Base<HistoryEntity> {
    ...
    @Query(value = "SELECT * FROM history WHERE createdAt >= :start AND createdAt < :end ORDER BY createdAt DESC")
    suspend fun getBetween(start: LocalDateTime, end: LocalDateTime): List<HistoryEntity>

    @Query(value = "SELECT COUNT(*) FROM history WHERE createdAt >= :start AND createdAt < :end")
